...
"""
Simple web server using BaseHTTPRequestHandler.

Routes:
- /            → index.html
- /message     → message.html (form)
- /read        → dynamic page with all messages (read.html)
- /static/...  → static files (css, images)
- 404 fallback → error.html

POST to /message stores data to storage/data.json
...
class RequestHandler(BaseHTTPRequestHandler):
    """
    Handles GET and POST HTTP requests for the web app.
    Supports serving static and dynamic content, storing and rendering messages.
    """
    ...
    def serve_static(self, file_path: Path) -> None:
        """Serve static files such as CSS, images, or JS."""
    ...
    def store_message(self, data: dict) -> None:
        """Store a new message in storage/data.json with timestamp."""
    ...
    def render_messages_page(self) -> None:
        """Render the read.html template with all stored messages."""
        try:
            messages = json.loads(FILE_PATH.read_text(encoding="utf-8"))
        except (FileNotFoundError, json.JSONDecodeError):
            messages = {}
